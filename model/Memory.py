...
#                     query_update[i] = torch.sum(query[idx].squeeze(1), dim=0)
    ...
    def get_score(self, mem, query):
        # get size of queries and memory elements
        bs, h, w, d = query.size()
        m, d = mem.size()
        
        score = torch.matmul(query, torch.t(mem))  # multiply to get a correlation matrix with shape of b X h X w X m
        score = score.view(bs*h*w, m)  # reshape matrix into shape of (b X h X w) X m

        # apply softmax function to get
    ...
    def forward(self, query, keys, train=True):
        ...
        # test
            ...
            # gathering loss - Feature compactness loss
            gathering_loss = self.gather_loss(query, keys, train)
            # read
            updated_query, softmax_score_query, softmax_score_memory = self.read(query, keys)
            # update
