...
class Memory(nn.Module):
    ...
    #                   (self, keys,gathering,   updating,       softmax_score_query, query_reshape, train)
    def get_update_query(self, mem, max_indices, update_indices, score, query, train):
        m, d = mem.size()
        if train:
            query_update = torch.zeros((m, d)).cuda()  # create a zeros tensor
            # random_update = torch.zeros((m,d)).cuda()
            for i in range(m):
                idx = torch.nonzero(max_indices.squeeze(1) == i)  # get index of nonzero item
                a, _ = idx.size()
                if a != 0:
    ...
    def get_score(self, mem, query):
        ...
        t_mem = torch.t(mem)
        score = torch.matmul(query, t_mem)  # b X h X w X m
    ...
    def update(self, query, keys, train):
        ...
        # calculate weights for update process
        softmax_score_query, softmax_score_memory = self.get_score(keys, query)
        # reshape query
        query_reshape = query.contiguous().view(batch_size*h*w, dims)

        # Return the largest elements of the given input score matrix
